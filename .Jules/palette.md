...
## 2026-06-15 - Graceful Degradation and Semantic Landmarks
**Learning:** In highly JavaScript-dependent applications (like fetching JSON or Markdown client-side), users with JS disabled or failed network requests often stare at infinite loading spinners or completely blank screens without explanation. Implementing a `<noscript>` fallback prevents this "silent failure". Additionally, wrapping filter controls in the HTML5 `<search>` element automatically provides a `search` ARIA landmark, greatly enhancing navigational structure for screen reader users without requiring custom `role` attributes.
**Action:** Always provide a robust `<noscript>` state that explains the JS dependency and forcefully hides infinite loading UI. Use semantic HTML like the `<search>` tag instead of generic `<div>`s for all filtering and search interfaces to organically improve screen reader landmarks.
