...
## 2026-06-07 - Cross-Browser Search Clear Buttons and Focus Management
**Learning:** Native browser clear buttons on `<input type="search">` (like `::-webkit-search-cancel-button`) render inconsistently across browsers and can't be fully styled for a cohesive UX. Furthermore, when replacing them with custom DOM buttons, hiding the custom clear button dynamically when the input becomes empty can result in a loss of keyboard focus if not handled correctly. If the focused element (the clear button) disappears, focus often resets to the document `<body>`, disrupting keyboard navigation.
**Action:** To ensure cross-browser consistency and proper accessibility for search inputs, hide native browser clear buttons (e.g., `::-webkit-search-cancel-button`) and replace them with custom `<button type="button">` elements. Manage their visibility via CSS (`:not(:placeholder-shown) ~ .clear-btn`). Critically, when the button is clicked to clear the input, explicitly return focus to the primary search input using `.focus()` to prevent focus traps and maintain the user's place in the interface.
